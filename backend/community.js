...
async function listTopics({ categoryId, search, cursor, status = 'all', sort = 'active', limit = 15, uid }) {
  ...
  const simplePage=sort==='active'&&status==='all';const snap=await q.limit(simplePage?n:40).get();let docs=snap.docs;if(uid){const blocked=await db().collection(C.blocks).where('ownerId','==',uid).limit(200).get();const ids=new Set(blocked.docs.map(x=>x.get('blockedUserId')));docs=docs.filter(x=>!ids.has(x.get('userId')));}if(status==='resolved')docs=docs.filter(x=>x.get('resolved')===true);else if(status==='open')docs=docs.filter(x=>x.get('resolved')!==true);if(sort==='newest')docs.sort((a,b)=>(b.get('createdAt')?.toMillis?.()||0)-(a.get('createdAt')?.toMillis?.()||0));else if(sort==='popular')docs.sort((a,b)=>(Number(b.get('likeCount'))||0)-(Number(a.get('likeCount'))||0)||(Number(b.get('viewCount'))||0)-(Number(a.get('viewCount'))||0));docs=docs.slice(0,n);return { items:docs.map(topicJson), nextCursor:simplePage&&snap.size>=n?cursorFrom(snap.docs.at(-1)):null };
}
async function topicDetail(uid, id, replyCursor) {
  ...
  const [fresh,vote,e,author]=await Promise.all([snap.ref.get(),snap.ref.collection('votes').doc(uid).get(),entitlement(uid),profile(d.userId)]);const topic={...topicJson(fresh),authorJoinedAt:author.joinedAt,authorMemberLevel:author.memberLevel,viewerVote:vote.exists?vote.get('value'):0};
