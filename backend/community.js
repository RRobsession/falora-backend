...
async function profile(uid) {
  ...
  return { displayName: clean(d.displayName || d.name || 'Meclis Üyesi').slice(0, 50), avatarUrl: clean(d.avatarUrl || d.photoUrl), role: d.communityRole === 'verified_reader' ? 'verified_reader' : 'member' };
}
...
async function topicDetail(uid, id, replyCursor) {
  ...
  const e=await entitlement(uid); if(!e.active) return {topic:topicJson(snap),entitlement:e,repliesLocked:true,replies:[],nextReplyCursor:null};
  ...
  const replies=await q.get(); return {topic:topicJson(snap),entitlement:e,repliesLocked:false,replies:replies.docs.map(x=>({id:x.id,...x.data(),createdAt:x.get('createdAt')?.toMillis?.()||null,editedAt:x.get('editedAt')?.toMillis?.()||null})),nextReplyCursor:replies.size===20?String(replies.docs.at(-1).get('createdAt').toMillis()):null};
}
...
async function createTopic(uid, body) {
  ...
  await ref.set({title,body:text,categoryId,categoryName:cat.get('name'),userId:uid,authorDisplayName:p.displayName,authorAvatarUrl:p.avatarUrl,authorRole:p.role,createdAt:admin.firestore.FieldValue.serverTimestamp(),lastActivityAt:admin.firestore.FieldValue.serverTimestamp(),expiresAt,moderationStatus:'published',replyCount:0,verifiedReaderReplyCount:0,resolved:false,acceptedAnswerId:null,reportCount:0,visibility:'public',locked:false,imageUrls:[],searchKeywords:keywords(title,text)});
  return {id:ref.id};
}
async function createReply(uid,id,body) {
  ...
  await db().runTransaction(async tx=>{tx.set(replyRef,{topicId:id,authorId:uid,authorDisplayName:p.displayName,authorAvatarUrl:p.avatarUrl,authorRole:p.role,body:text,createdAt:admin.firestore.FieldValue.serverTimestamp(),editedAt:null,moderationStatus:'published',helpfulCount:0,isAcceptedSolution:false,reportCount:0,visibility:'public'});tx.update(topicRef,{replyCount:admin.firestore.FieldValue.increment(1),verifiedReaderReplyCount:admin.firestore.FieldValue.increment(p.role==='verified_reader'?1:0),lastActivityAt:admin.firestore.FieldValue.serverTimestamp()});}); return {id:replyRef.id,topicOwnerId:topic.get('userId'),verifiedReader:p.role==='verified_reader'};
}
async function acceptSolution(uid,topicId,replyId) {
  await requirePremium(uid); const t=db().collection(C.topics).doc(topicId), r=t.collection('replies').doc(replyId);
  let answerAuthorId=null; await db().runTransaction(async tx=>{const [ts,rs]=await Promise.all([tx.get(t),tx.get(r)]);if(!ts.exists||ts.get('userId')!==uid) fail('Yalnızca konu sahibi çözüm seçebilir.',403);if(!rs.exists)fail('Cevap bulunamadı.',404);answerAuthorId=rs.get('authorId');const old=ts.get('acceptedAnswerId');if(old&&old!==replyId)tx.set(t.collection('replies').doc(old),{isAcceptedSolution:false},{merge:true});tx.update(r,{isAcceptedSolution:true});tx.update(t,{acceptedAnswerId:replyId,resolved:true});}); return {success:true,answerAuthorId};
}
...
async function cleanupExpired(){const s=await db().collection(C.topics).where('expiresAt','<=',admin.firestore.Timestamp.now()).limit(100).get();let count=0;for(const doc of s.docs){const paths=doc.get('storagePaths')||[];if(paths.length){const bucket=admin.storage().bucket(process.env.COMMUNITY_STORAGE_BUCKET||process.env.FIREBASE_STORAGE_BUCKET||'tombikteyze.appspot.com');await Promise.all(paths.map(p=>bucket.file(p).delete({ignoreNotFound:true}).catch(()=>null)));}const replies=await doc.ref.collection('replies').limit(500).get();const batch=db().batch();replies.docs.forEach(x=>batch.delete(x.ref));batch.delete(doc.ref);await batch.commit();count++;}return{deleted:count};}
...
module.exports={C,categories,listTopics,topicDetail,createTopic,createReply,acceptSolution,report,block,attachImages,entitlement,adminOverview,adminList,adminTopicDetail,adminReply,adminReplyAction,adminGrantTestEntitlement,adminModerate,adminUserAction,cleanupExpired,startCleanupLoop,normalize,moderate,limits};
